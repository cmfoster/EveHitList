class EveApiChecker 
  ...
  def self.check_corp_journal_for_new_donations(test_time=nil)
    ...
    outlaw_names = entries.collect {
      |e| [e.reason.gsub(/[DECS:\n\r]/, ""), e.balance] if e.refTypeID.to_i == 10 && e.date.to_time > (test_time || @@last_request_time)
    }
  ...
  def self.log_api_access_time(time=nil)
  ...
  def self.get_character_id(acct, outlaw_names)
    acct.scope = "eve"
    begin
      temp_coll = acct.characterID(:names => outlaw_names.join(',')).characters
    rescue
      return false
    end
    ...
    combine.each do |target|
      WantedToon.make_wanted_toon(target[0],target[1],target[2])
    end
  end
